<template>
  <div class="pt-6" :class="$route.path === '/pricing' || $route.path === '/careers' ? 'bg-[#F7F6F3]' : 'bg-none'">
    <div>
        ...
        <div class="flex items-center flex-1">
          <div class="flex items-center justify-between w-full md:w-auto">
            <NuxtLink to="/" class="pb-1">
              ...
              <img class="h-6 w-auto sm:h-8" src="@/assets/images/logo-wide.svg" alt="" />
            </NuxtLink>
            <div class="-mr-2 flex items-center md:hidden">
              <button @click="openNavbar = true"
          ...
          <div class="hidden md:block md:ml-10 md:space-x-12">
            <NuxtLink v-for="item in navigation" :key="item.name" :to="item.to" exact-active-class="opacity-100"
      ...
      <transition enter-active-class="duration-150 ease-out" enter-from-class="opacity-0 scale-95"
        ...
        <div v-if="openNavbar" focus
          class="absolute z-10 top-0 inset-x-0 p-2 transition transform origin-top-right md:hidden">
          <div class="rounded-xl shadow-md bg-white ring-1 ring-black ring-opacity-5 overflow-hidden">
            ...
            <div class="px-2 pt-2 pb-3 space-y-1">
              <button v-for="item in navigation" :key="item.name" @click="goTo(item.to)"
